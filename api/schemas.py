...
class PatientInputBasic(BaseModel):
    ...
    high_bp: bool = Field(..., description="Đã từng được bác sĩ chẩn đoán cao huyết áp?")
    high_chol: bool = Field(..., description="Đã từng được bác sĩ chẩn đoán cholesterol cao?")
    chol_check: bool = Field(True, description="Có kiểm tra cholesterol trong 5 năm qua?")
    ...
    phys_activity: bool = Field(..., description="Có tập thể dục/vận động trong 30 ngày qua?")
    ...
    mental_health_days: int = Field(
        0, ge=0, le=30, description="Số ngày cảm thấy không ổn về tinh thần trong 30 ngày qua"
    )
    phys_health_days: int = Field(
        0, ge=0, le=30, description="Số ngày cảm thấy không khỏe về thể chất trong 30 ngày qua"
    )
    diff_walk: bool = Field(..., description="Khó khăn khi đi bộ hoặc leo cầu thang?")

    any_healthcare: bool = Field(True, description="Có bảo hiểm y tế/tiếp cận dịch vụ y tế?")
...
class PredictionResponse(BaseModel):
    prediction: int = Field(..., description="0 = không mắc bệnh tim, 1 = có mắc bệnh tim")
...
class ChatMessage(BaseModel):
    role: Literal["user", "assistant"] = Field(..., description="Vai trò trong hội thoại")
    content: str = Field(..., description="Nội dung tin nhắn")


class ChatRequest(BaseModel):
    mode: Literal["basic", "advanced"] = Field(..., description="Chế độ đã dùng để dự đoán")
    patient: dict = Field(..., description="Dữ liệu bệnh nhân đã gửi lúc /predict (nguyên payload)")
    prediction_result: PredictionResponse = Field(..., description="Kết quả trả về từ /predict")
    message: str = Field(..., min_length=1, description="Câu hỏi/tin nhắn mới của người dùng")
    history: list[ChatMessage] = Field(
        default_factory=list, description="Lịch sử hội thoại trước đó (không gồm message mới nhất)"
    )
